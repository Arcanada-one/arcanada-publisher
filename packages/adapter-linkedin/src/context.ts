...
// Mirrors adapter-facebook/context.ts — kept per-adapter (not in core) so each
// adapter can tune launch args / UA / viewport without coupling.
...
export async function launchSession(options: LaunchOptions): Promise<BrowserSession> {
  // PUB-0033: force the LinkedIn UI to English and suppress the Chrome translate
  // bar. LinkedIn renders its chrome in the account/session display language (we
  // have seen RU, EN, DE, FI); pinning locale + Accept-Language to en-US makes
  // the selectors deterministic. When the feed is still served in another
  // language (edge cache), Chrome's built-in translate popup would otherwise
  // appear over the composer — the --disable-features=Translate flag plus the
  // translate prefs below turn it off so it never overlaps an automated click.
