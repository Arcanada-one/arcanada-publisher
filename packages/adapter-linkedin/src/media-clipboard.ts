...
export function pasteMediaClipboardNative(
  deps: NativePasteDeps = { platform: process.platform, exec: execFileSync },
): boolean {
  ...
  try {
    deps.exec("osascript", ["-e", script], { encoding: "utf8" });
    return true;
  } catch {
    throw new AdapterError(ErrorCode.VERIFY_FAILED, "macOS native media paste failed", {
      stage: "media_clipboard_paste",
    });
  }
}

export function focusPublisherBrowserNative(
  deps: NativePasteDeps = { platform: process.platform, exec: execFileSync },
): boolean {
  if (deps.platform !== "darwin") return false;
  const script = [
    'tell application "System Events"',
    'repeat with browserName in {"Google Chrome for Testing", "Chromium", "Google Chrome"}',
    'set processName to browserName as text',
    'if exists process processName then',
    'set frontmost of process processName to true',
    'return "ok"',
    'end if',
    'end repeat',
    'error "publisher browser process not found"',
    'end tell',
  ].join("\n");
  try {
    deps.exec("osascript", ["-e", script], { encoding: "utf8" });
    ...
    throw new AdapterError(ErrorCode.VERIFY_FAILED, "macOS Publisher browser focus failed", {
      stage: "media_clipboard_focus",
    });
  }
}
