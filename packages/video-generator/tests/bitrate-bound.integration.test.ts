...
function probeOutput(filePath: string): ProbeResult {
  ...
  const fmtOut = execFileSync(
    ffprobeBin,
    ["-v", "error", "-show_entries", "format=duration,bit_rate", "-of", "flat", filePath],
    { encoding: "utf8" },
  );
  ...
  const streamOut = execFileSync(
    ffprobeBin,
    ["-v", "error", "-show_entries", "stream=codec_type,pix_fmt", "-of", "flat", filePath],
    { encoding: "utf8" },
  );
  ...
  try {
    const traceOut = execFileSync(ffprobeBin, ["-v", "trace", filePath], {
      encoding: "utf8",
    ...
    try {
      const result = execFileSync(ffprobeBin, ["-v", "trace", filePath], {
        encoding: "utf8",
        stdio: ["pipe", "pipe", "pipe"],
      });
...
function makeHighMotionCover(workdir: string): string {
  ...
  execFileSync(
    ffmpegBin,
    ["-y", "-v", "error", "-f", "lavfi", "-i", "mandelbrot=size=1280x720", "-vframes", "1", path],
    { encoding: "utf8" },
  );
  return path;
}
...
describeIntegration(
  "T7: cycle preset at default 600 kbps cap",
  () => {
    let outPath: string;
...
describeIntegration(
  "T8: explicit 300 kbps cap reduces bitrate vs default",
  () => {
